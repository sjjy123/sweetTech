...
<?php 
	...
	if ($_REQUEST["suname"]!=""){$suname=urldecode($_REQUEST["suname"]);}
	...
	if($suname!=''){
		$count_sql=$count_sql." and user_name like '%$suname%'";
	}
...
//		}else{
	...
	if ($suname!="") {
			$Cms_sql=$Cms_sql." and user_name like  '%$suname%' ";
	}
	//echo $Cms_sql;
	//exit;
  ...
  while ($Cms_Row=$db->fetch_array($Cms_result))
  {
     ...
     echo "<tr>";
       echo "<td class='$str_classname'><input name='ids[]' id='ids' type='checkbox'  value=".$Cms_Row["id"]." />".$Cms_Row["id"]."</td>";     
		  ...
		  switch ($field_arr[$x][1]) //fieldout的值
       	  {
				...
				elseif($field_arr[$x][0]=="datetime" || $field_arr[$x][0]=="traveltime" )
  ...
  echo "<table id='tablehovered1' border='0' align='center' cellpadding='2' cellspacing='2' width=100%>";
	  ...
	  echo "<tr><td width=20%>";
	  echo "选择： <span style='cursor:pointer;' onclick='return viode(0)'>全部</span> - ";
	  ...
	  echo "<span><input type='radio' name='actn' id='actn' value='del' />删除</span>";
	  echo "&nbsp;<span><input type='radio' name='actn' id='actn' value='rec' />回收</span>";
	  
	  if(isshow($SystemID,"classid")==true)
	  {
		  echo "&nbsp;<span><input type='radio' name='actn' id='actn' value='move' />移动</span>";
    ...
    echo "&nbsp;&nbsp;<b>关键字</b>：<input type='text' name='Keyword' id='Keyword' style='vertical-align:middle' />"; 
    echo "&nbsp;&nbsp;<b>帐号</b>：<input type='text' name='suname' id='suname' style='vertical-align:middle' />"; 
    echo  "&nbsp;&nbsp;<input type='submit' name='button' id='button' value='确认搜索' style='vertical-align:middle' />";
    echo "</form>";
    echo "</td>"; //搜索结束
...
<script language="javascript">
  ...
  {   
	...
	function ok(smbt){
		...
		{
			$.get("../sys/admin_stemp.php?t="+new Date()+")",{action:'stemp',field:stemp,id:id,tabname:tabname,val:value},function(responseHTML){
