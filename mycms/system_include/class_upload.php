		...
		if(isset($_FILES[$fileInput])){
			...
			}else
			{
				...
				if($this->copyfile($fileArr)){
					//$this->returnArray[] =  $this->returninfo;
					global $fieldname,$fieldout;
					if($fieldout!=9){echo "<script>parent.document.getElementById(\""."s_".$fieldname."\").value=\"".$this->thumbPrefix.$this->saveName."\"</script>";}
					echo "<script>parent.document.getElementById(\"$fieldname\").value=\"".$this->saveName."\";parent.document.getElementById(\""."yc_".$fieldname."\").src=\"uploadfile/".substr($this->saveName,'0','7')."/".$this->saveName."\"</script>";
					//echo "<script>parent.document.getElementById(\"$fieldname\").value=\"".$file_name."\"< /script>";
					$this->errno = 15;
				}else{
					$this->returninfo['error'] = $this->errmsg();
					//$this->returnArray[] =  $this->returninfo;
...
// @param $saveName 保存名，如果为空，则系统自动生成一个随机的文件名
	function setSavename($saveName){
		if ($saveName == ''){  // 如果未设置文件名，则生成一个随机文件名
			$name = date("Y-m-d").mt_rand(1000,9999).'.'.$this->ext;
		} else {
			$name = $saveName;
		}
		$this->saveName = $name;
...
// 得到错误信息
	function errmsg(){
		$uploadClassError = array(
			...
			10	=>'输入的名称不可用!<a href="'.$_SERVER['HTTP_REFERER'].'">返回</a>',
			11	=>'不允许上传的文件类型!<a href="'.$_SERVER['HTTP_REFERER'].'">返回</a>',
			12	=>'没有文件夹写入权限，请联系服务器管理员!',
			13	=>'同名文件已经存在!<a href="'.$_SERVER['HTTP_REFERER'].'">返回</a>',
			14	=>'文件超过限制大小!<a href="'.$_SERVER['HTTP_REFERER'].'">返回</a>',
			//15	=>'Delete file unsuccessfully!',
			15	=>'上传成功!<a href="'.$_SERVER['HTTP_REFERER'].'">重传</a>',
			...
			23  =>'上传成功!<a href="'.$_SERVER['HTTP_REFERER'].'">重传</a>',
			);
		if ($this->errno == 0)
			return false;
		else
