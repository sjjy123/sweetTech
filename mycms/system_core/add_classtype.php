...
?>
    ...
    <td class="tablerow1">
    <INPUT name="m" value="<?php echo $SystemID;?>" type="hidden">
    <input name="ClassName" type="text" id="ClassName" value="" size="35"><span class="Warning"><font id="ClassNameHTML"></font></span></td>
    </tr>
    <tr>
    <td class="tablerow2" align="right"><u title="MainDomain"><?php if($m==81||$m==79){echo '地址';}else{echo '英文名称';}?></u>：</td>
    ...
    <?php
     ...
     elseif(addslashes($_REQUEST['act'])=='edit')
     {
        ...
        */
        $Result=$db->query("select partid,ClassName,ClassNameEng,ClassInfo,sortpic,`show` from ".$tablepre."class where ID=$ID");
    ...
    echo "<option value='0'>作为一级类</option>";
    $ClaSql="Select * from ".$tablepre."class where SystemID=$SystemID  order by sequence desc";
    $Result=$db->query($ClaSql);
    while ($Row=$db->fetch_array($Result)) 
    {
    //通过分拆、组合 获得一个新的数组排序的值------开始
	...
	{
		$rs=$db->fetch_array($db->query("select * from ".$tablepre."class where ID=$ClssID"));
    ...
    $CmsSql="insert into ".$tablepre."class (`ClassName`,`ClassNameEng`,`ClassInfo`,`Depth`,`ParPath`,`partid`,`SystemID`,`Sequence`,`CmsDate`,`sortpic`,`show`,`ycpaixu`) VALUES ('$ClassName','$ClassNameEng','$ClassInfo','$Depth','$ParPath','$ClssID','$SystemID','$Sequence','$CmsDate','$sortpic','$show','$yc_paixu')";
    $Result=$db->query($CmsSql);
    if ($ClssID!=0)
    {
        $db->query("update ".$tablepre."class set Num=Num+1 where ID=$ClssID");
    }
    ok("信息添加成功","add_classtype.php?act=add&m=$SystemID",2);
    }
    ...
    */
    ...
    if($_FILES['sortpic']['name']!="") //有上传文件的情况=====================================================
    {
	  ...
	  $mysql=$db->query("select * from ".$tablepre."class where ID=$ID");
    ...
    $CmsSql="update ".$tablepre."class set ClassName ='$ClassName',ClassNameEng='$ClassNameEng',sortpic='$sortpic',`show`='$show',ClassInfo='$ClassInfo',Depth='$Depth',ParPath='$ParPath',partid='$ClssID',SystemID='$SystemID' where ID=$ID";
    $db->query($CmsSql);
    ok("信息编辑成功","admin_class.php?m=$SystemID",2);
    }
    /*
    更新分类信息 End
    */	
