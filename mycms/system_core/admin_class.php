...
include_once(INIT_ROOT."system_include/check_purview.php");
	...
	if(addslashes($_REQUEST['act'])=='del') {
 		$db->query("delete from ".$tablepre."class where ID=$ID");
		header("location:admin_class.php?m=$SystemID");
    }
...
<table id="tablehovered" align="center" class="tableborder" cellspacing="1" cellpadding="2">
  ...
  <?php
	if ($_SESSION["adminLov"]>=1) { 
		$Cmsql = "Select * from ".$tablepre."class where SystemID=$SystemID order by Sequence Asc";
	}else{
		$Cmsql = "Select * from ".$tablepre."class where SystemID=$SystemID order by Sequence Asc";
	}
	//echo $Cmsql;
	...
	if ($_SESSION["adminLov"]>=1) { 
		ShowClass(0,true,$SystemID);
	}else{
		//我当前有的分类权限adminclassKj
		ShowClass2(0,true,$SystemID);
	}
	//输出分类内容  
 ?>
</table>
 ...
 */  
 if(addslashes($_REQUEST["act"])=="upd")
 {
 	...
 	if(is_numeric(addslashes($_REQUEST["sequeceID"])))//判断是否为数字
 	{
		...
		$ID =addslashes($_REQUEST["ID"]);
		$sel_sql=$db->query("select * from ".$tablepre."class where ID=".$ID);
		...
		//生成自身的排序值--结束
		$UpSql="update ".$tablepre."class set sequence=$sequeceID where ID=$ID";
 		$db->query($UpSql);
		$UpSql="update ".$tablepre."class set ycpaixu='$sequeceID_paixu' where ID=$ID";
 		$db->query($UpSql);
		cms_get_digui_sequence($ID,$sequeceID_paixu);//子分类的排序值修改
		echo "<script language=\"JavaScript\">";
  		echo "location.replace(\"admin_class.php?m=$SystemID\")";
  		echo "</script>";
 	}
 	else
 	{
